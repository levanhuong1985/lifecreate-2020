...
add_image_size( 'thumb-news', 320, 237, true );
add_image_size( 'thumb-cruit',272, 204, true );
...
function login_logo() {    
    ...
    .login h1 a {
          background-image: url('.HOME_URL.'/imgs/common/logo.svg);
          background-size:100%;
		  width:246px;
...
add_filter('previous_post_link', 'post_link_attributes_pre');
...
function post_link_attributes_pre($output) {
    $code = 'class="post-prev"';
    return str_replace('<a href=', '<a '.$code.' href=', $output);
}
